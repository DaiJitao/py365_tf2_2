...
config = {
    ...
    "first_order": True, # 计算一节特征时，一节特征是否需要embedding
}
...
# 2.先求和再平方
sum_kd_embed = Lambda(lambda x: K.sum(x, axis=1), name='second_order_sum')(concat_sparse_kd_embed)  # ?, k
...
deep_inputs = Concatenate(axis=1, name='sparEmb_denseIn_con')([flatten_sparse_embed, concat_dense_inputs]) # (?, n*k + desne_feat_size)
...
if __name__ == '__main__':
    ...
    if train:
        ...
        with strategy.scope():
            ...
            model.summary()
            # plot_model(model, show_shapes=True, to_file='deepfm.png')
        ...
        model.fit(train_data, verbose=1, steps_per_epoch=steps_per_epoch, epochs=epochs,
                  #validation_data=test_data, validation_batch_size=1,
                  callbacks=callbacks,
                  shuffle=True)
